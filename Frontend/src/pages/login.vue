...
const handleSubmit=async()=>{
 const result = await v$.value.$validate().then();
  ...
  ApiService.post("login",form.value).then((response)=>{
...
const loginWithFacebook=async()=>{
 router.replace("http://127.0.0.1:8000/auth/facebook/auth/facebook")
}
</script>
        ...
        <VForm @submit.prevent="handleSubmit">
          <VRow>
            ...
            <VCol
              ...
              class="text-center"
            >
              <AuthProvider @loginWithSocial="loginWithFacebook"/>
            </VCol>
          </VRow>
        </VForm>
      </VCardText>
